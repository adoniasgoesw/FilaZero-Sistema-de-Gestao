...
const FormFecharCaixa = ({ onSubmit, onCancel, openingValue = 0 }) => {
  const denominations = [100, 50, 20, 10, 5, 2];
  const [valorFechamentoBase, setValorFechamentoBase] = useState("");
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onSubmit?.(calculatedClosing);
  };

  return (
    <form className="space-y-6" onSubmit={handleSubmit}>
      <h3 className="text-2xl font-bold text-[#1A99BA]">Fechar caixa</h3>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Valor de fechamento (manual)</label>
          <input
            type="number"
            step="0.01"
            value={valorFechamentoBase}
            onChange={(e)=>setValorFechamentoBase(e.target.value)}
